...
export default class Test extends BasicTest
{
    ...
    async _crypto_generichash_Test() {
        ...
        let results = sample_crypto_generichash.map(t => {
            let q = Sodium.crypto_generichash_sync(64, Base64.fromByteArray(this.hex2bin(t.value)), "");
            try {
                let response = JSON.parse(q);
                return t.hash === this.bin2hex(Base64.toByteArray(response.data));
            } catch (e) {
                return false;
            }
        });
        this.testPassed('crypto_generichash_sync', results.find(t => t === false) === undefined);
    }
    componentDidMount() {
        this.initTests([
            'crypto_generichash_sync'
        ])
