...
export default class Test extends BasicTest
{
    ...
    async _crypto_core_Test() {
        ...
        let results: Array;
        results = sample_crypto_core_ed25519_random.map(t => {
            let q = Sodium.crypto_core_ed25519_random_sync();
            try {
                let response = JSON.parse(q);
                return Base64.toByteArray(response.data).length === 32;
            } catch (e) {
                return false;
            }
        });
        this.testPassed('crypto_core_ed25519_random_sync', results.find(t => t === false) === undefined);

        results = sample_crypto_core_ed25519_random.map(t => {
            let q = Sodium.crypto_core_ed25519_from_uniform_sync(Base64.fromByteArray(this.hex2bin(t.random)));
            try {
                let response = JSON.parse(q);
                return t.crypto_core_ed25519_from_uniform === this.bin2hex(Base64.toByteArray(response.data));
            } catch (e) {
                return false;
            }
        });
        this.testPassed('crypto_core_ed25519_from_uniform_sync', results.find(t => t === false) === undefined);

        results = sample_crypto_core_ed25519_add_sub.map(t => {
            let q = Sodium.crypto_core_ed25519_add_sync(Base64.fromByteArray(this.hex2bin(t.p)), Base64.fromByteArray(this.hex2bin(t.q)));
            try {
                let response = JSON.parse(q);
                return t.add === this.bin2hex(Base64.toByteArray(response.data));
            } catch (e) {
                return false;
            }
        });
        this.testPassed('crypto_core_ed25519_add_sync', results.find(t => t === false) === undefined);

        results = sample_crypto_core_ed25519_add_sub.map(t => {
            let q = Sodium.crypto_core_ed25519_sub_sync(Base64.fromByteArray(this.hex2bin(t.p)), Base64.fromByteArray(this.hex2bin(t.q)));
            try {
                let response = JSON.parse(q);
                return t.sub === this.bin2hex(Base64.toByteArray(response.data));
            } catch (e) {
                return false;
            }
        });
        this.testPassed('crypto_core_ed25519_sub_sync', results.find(t => t === false) === undefined);

        results = sample_crypto_core_ed25519_scalar_reduce.map(t => {
            let q = Sodium.crypto_core_ed25519_scalar_reduce_sync(Base64.fromByteArray(this.hex2bin(t.hash)));
            try {
                let response = JSON.parse(q);
                return t.result === this.bin2hex(Base64.toByteArray(response.data));
            } catch (e) {
                return false;
            }
        });
        this.testPassed('crypto_core_ed25519_scalar_reduce_sync', results.find(t => t === false) === undefined);


        results = sample_crypto_core_ed25519_scalar_xxxxx.map(t => {
            let q = Sodium.crypto_core_ed25519_scalar_add_sync(Base64.fromByteArray(this.hex2bin(t.x)), Base64.fromByteArray(this.hex2bin(t.y)));
            try {
                let response = JSON.parse(q);
                return t.add === this.bin2hex(Base64.toByteArray(response.data));
            } catch (e) {
        ...
        this.testPassed('crypto_core_ed25519_scalar_add_sync', results.find(t => t === false) === undefined);

        results = sample_crypto_core_ed25519_scalar_xxxxx.map(t => {
            let q = Sodium.crypto_core_ed25519_scalar_sub_sync(Base64.fromByteArray(this.hex2bin(t.x)), Base64.fromByteArray(this.hex2bin(t.y)));
            try {
                let response = JSON.parse(q);
                return t.sub === this.bin2hex(Base64.toByteArray(response.data));
            } catch (e) {
        ...
        this.testPassed('crypto_core_ed25519_scalar_sub_sync', results.find(t => t === false) === undefined);

        results = sample_crypto_core_ed25519_scalar_xxxxx.map(t => {
            let q = Sodium.crypto_core_ed25519_scalar_mul_sync(Base64.fromByteArray(this.hex2bin(t.x)), Base64.fromByteArray(this.hex2bin(t.y)));
            try {
                let response = JSON.parse(q);
                return t.mul === this.bin2hex(Base64.toByteArray(response.data));
            } catch (e) {
                return false;
            }
        });
        this.testPassed('crypto_core_ed25519_scalar_mul_sync', results.find(t => t === false) === undefined);

        results = sample_crypto_core_ed25519_scalar_xxxxx.map(t => {
            let q = Sodium.crypto_core_ed25519_scalar_negate_sync(Base64.fromByteArray(this.hex2bin(t.x)));
            try {
                let response = JSON.parse(q);
                return t.negate_x === this.bin2hex(Base64.toByteArray(response.data));
            } catch (e) {
                return false;
            }
        });
        this.testPassed('crypto_core_ed25519_scalar_negate_sync', results.find(t => t === false) === undefined);

        results = sample_crypto_core_ed25519_scalar_xxxxx.map(t => {
            let q = Sodium.crypto_core_ed25519_scalar_invert_sync(Base64.fromByteArray(this.hex2bin(t.x)));
            try {
                let response = JSON.parse(q);
                return t.invert_x === this.bin2hex(Base64.toByteArray(response.data));
            } catch (e) {
                return false;
            }
        });
        this.testPassed('crypto_core_ed25519_scalar_invert_sync', results.find(t => t === false) === undefined);

        results = sample_crypto_core_ed25519_scalar_xxxxx.map(t => {
            let q = Sodium.crypto_core_ed25519_scalar_complement_sync(Base64.fromByteArray(this.hex2bin(t.x)));
            try {
                let response = JSON.parse(q);
                return t.complement_x === this.bin2hex(Base64.toByteArray(response.data));
            } catch (e) {
                return false;
            }
        });
        this.testPassed('crypto_core_ed25519_scalar_complement_sync', results.find(t => t === false) === undefined);
    }

    componentDidMount() {
        this.initTests([
            'crypto_core_ed25519_random_sync',
            'crypto_core_ed25519_from_uniform_sync',
            'crypto_core_ed25519_add_sync',
            'crypto_core_ed25519_sub_sync',
            'crypto_core_ed25519_scalar_reduce_sync',
            'crypto_core_ed25519_scalar_add_sync',
            'crypto_core_ed25519_scalar_sub_sync',
            'crypto_core_ed25519_scalar_mul_sync',
            'crypto_core_ed25519_scalar_negate_sync',
            'crypto_core_ed25519_scalar_invert_sync',
            'crypto_core_ed25519_scalar_complement_sync'
        ])
